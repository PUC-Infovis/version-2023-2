...
// Carga de datos
const data_conferencia = d3.json('data/arenas.geojson');
const data_palmares = d3.dsv(';', 'data/PALMARES_NBA_NUEVO_.csv');
const data_season = d3.csv('data/teams_nba.csv');
...
Promise.all([data_conferencia, data_palmares, data_season]).then(function (data) {
  ...
  d3.json('data/us-states.json').then(function (us) {
    ...
    d3.json('data/arenas.geojson').then(function (geo_teams) {
      d3.dsv(';', 'data/PALMARES_NBA_NUEVO_.csv').then(function (teams) {
